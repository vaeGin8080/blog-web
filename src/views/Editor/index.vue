	...
	<div class="wrap">
				...
				<div class="p-right h-100 flex justify-end align-center">
					<el-dropdown @command="handlePreview">
						<span class="el-dropdown-link">
							<i class="el-icon-s-promotion" style="color:white;margin-right:10px"></i>
						</span>
		...
		<div class="editor-main">
			<div class="wrap-left" :class="{ hide: !showLeft }">
				<!-- 公共节点 -->
				<draggable class="list-group" element="div" v-model="listLeft" :sort="false" :options="optionLeft"
					:move="onMove" @start="isDragging = true" @end="isDragging = false" dragClass="pullDragClass"
					chosenClass="pullChosenClass" ghostClass="pullGhostClass">
					<div v-for="(item, key) in listLeft" :key="key" class="o-item">
						<div class="item flex align-center">
							<img :src="item.chartImg" />
							{{ item.chartName }}
						</div>
					</div>
				</draggable>
			...
			<div class="wrap-editor" id="main">
				<div class="screen-editor" :style="option">
					<draggable class="editor-group" element="div" :sort="false" v-model="listRight"
						:options="optionEditor" :move="onMove" ghostClass="e-item" @start="rightStart" @end="rightEnd">
						<div v-for="(item, key) in listRight" :key="key">
							<sortChart :type="item.type || 'chart'" :index="key" :scale="scale" :config="item"
								@activated="handleActivated" @deactivated="handleDeactivated"
								@handleResize="handleResize"></sortChart>
						</div>
					</draggable>
				</div>
			</div>
		...
		methods: {
			initChart() {
				getChartList().then(res => {
					if (res.status == 1) {
						this.listLeft = res.data.data.map(item=>{
							return {
								...item,
								chartOption: eval("(" + item.chartOption + ")")
							}
						})
			...
			configChange(e) {
				console.log(e);
				this.$set(this.listRight, e.index, {
					...this.listRight[e.index],
					...e
				});
